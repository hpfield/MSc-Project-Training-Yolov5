...
class Detect:
    ...
    def detect(self):
        ...
        # Directories
        # save_dir = increment_path(Path(opt.project) / self.opt.name, exist_ok=opt.exist_ok)  # increment run
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                imc = im0.copy() if self.opt.save_crop else im0  # for self.opt.save_crop
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        item = ('%g ' * len(line)).rstrip() % line
                        result.append(item)

                    labels = [[float(num) for num in line.split()] for line in result]
                    for label in labels:
                        label[0] = int(label[0])
            ...
            #             label = None if self.opt.hide_labels else (names[c] if self.opt.hide_conf else f'{names[c]} {conf:.2f}')
